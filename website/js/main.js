...
(function () {
  ...
  /* ---------- Configurable game launcher ----------
     ...
     Same-origin URLs can be embedded in an iframe; anything else
  ...
  function showEmbed() {
    ...
    if (!gameFrame.querySelector('iframe')) {
      var f = document.createElement('iframe');
      f.setAttribute('src', window.GAME_URL);
      f.setAttribute('title', 'MUNDA wire-assembly game');
      f.setAttribute('loading', 'lazy');
      gameFrame.appendChild(f);
    }
    var openBtn = document.createElement('a');
    openBtn.className = 'gp-chip gp-chip--open';
    openBtn.setAttribute('href', window.GAME_URL);
    openBtn.setAttribute('target', '_blank');
    openBtn.setAttribute('rel', 'noopener');
    openBtn.textContent = '↗ OPEN GAME IN NEW TAB';
    if (!gameFrame.querySelector('.gp-chip--open')) gameFrame.appendChild(openBtn);
  }
  ...
  function launchGame(e) {
    ...
    if (window.USE_EMBEDDED_GAME && isSameOrigin(url)) {
      document.getElementById('game').scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth' });
      showEmbed();
    } else if (url && url !== 'YOUR_GAME_URL_HERE') {
      window.open(url, '_blank', 'noopener');
    }
  }

  var playButtons = Array.prototype.slice.call(document.querySelectorAll('#nav-play, #gp-launch, #cta-play'));
  playButtons.forEach(function (b) {
    b.addEventListener('click', launchGame);
  });
